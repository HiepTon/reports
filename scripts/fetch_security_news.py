    ...
    *,
    ...
    for it in items:
        ...
        if not key or key in seen:
            continue
        seen.add(key)
        parts.append(f"{len(parts) + 1}. {it.topic}: {(it.summary or '').strip()}")
    ...
    prompt = (
        "You are a security news editor. Below is today's FULL list of stories (number, title: summary). "
        "Write a BRIEFING in English that COVERS all the main developments, grouped by theme with one short "
        "paragraph per theme; do not omit significant stories and do not repeat duplicates. Neutral tone. "
        "Return plain text only (paragraphs separated by a blank line), no markdown, no heading.\n\n"
